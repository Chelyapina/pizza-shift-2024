...
fun PizzaAlert(
    pizza: OnePizzaInfo,
    onDismiss: () -> Unit,
    onAddToCart: (DataCartForOnePizza) -> Unit
) {
    val selectedSize = remember { mutableStateOf<Sizes?>(null) }
    val selectedDough = remember { mutableStateOf<Doughs?>(Doughs(name = NameDoughs.THIN, price = 0)) }
    val selectedToppings = remember { mutableStateListOf<DataToppings>() }

//    val cartState = remember { DataCartForOnePizza(
//        name = "",
//        dataToppings = listOf(),
//        description = "",
//        sizes = listOf(),
//        doughs = listOf(Doughs(name = NameDoughs.THIN, price = 0))
//    )}
                ...
                SingleChoiceSegmentedButtonRow(
                    ...
                    pizza.sizes?.forEachIndexed { index, size ->
                        SegmentedButton(
                            ...
                            onClick = { selectedIndex = index; selectedSize.value = size },
...
fun ToppingsCard(
    ...
    onToppingSelected: (DataToppings) -> Unit,
    onToppingDeselected: (DataToppings) -> Unit
) {
    ...
    Card(
        modifier = Modifier
            ...
            .clickable {
                ...
                if (isSelected) {
                    onToppingSelected(ingredient)
                } else {
                    onToppingDeselected(ingredient)
                }
            }
            .background(
